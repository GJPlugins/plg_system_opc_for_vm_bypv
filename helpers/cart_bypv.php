...
class VirtueMartCart_byPV
{
	...
	public static function Initialize()
	{
		...
		if ($VMCFG_ONCHECKOUT_ONLY_REGISTERED || VmConfig::get('oncheckout_show_register', 0) != 1)
		{
			$disable_customer_types[] = self::CT_REGISTRATION;
		}

		if ($VMCFG_ONCHECKOUT_ONLY_REGISTERED || plgSystemOPC_for_VM_byPV::isPluginParamEnabled('allow_order_from_guest') === FALSE)
		{
...
// 		$override_methods = array('redirect');
		$override_methods = array();
		if ($trigger_select_check !== TRUE) $override_methods[] = 'enqueueMessage';
	...
	public function fixShipment()
	{
		...
		{
			$this->shipment = $cart->virtuemart_shipmentmethod_id;
...
// 			$this->setUserFieldsData(self::UFT_BILLING_ADDRESS, NULL);
// 			$this->setUserFieldsData(self::UFT_BILLING_ADDRESS, $this->getUserFieldDefaultValues(self::UFT_BILLING_ADDRESS));
...
// 			}
			...
			vmError(
				'Invalid argument in function ' . __FUNCTION__ . '. Valid argument is constant CT_... from class ' . __CLASS__ . '.',
...
// 					$this->setUserFieldsData(self::UFT_SHIPPING_ADDRESS, NULL);
				...
				case self::ST_NEW_ADDRESS:
					...
					$cart->ST = $this->getUserFieldsData(self::UFT_SHIPPING_ADDRESS);
// 					$this->setUserFieldsData(self::UFT_SHIPPING_ADDRESS, NULL);
// 					$this->setUserFieldsData(self::UFT_SHIPPING_ADDRESS, $this->getUserFieldDefaultValues(self::UFT_SHIPPING_ADDRESS));
					break;
			}
		...
		{
			vmError(
				'Invalid argument in function ' . __FUNCTION__ . '. Valid argument is constant ST_... from class ' . __CLASS__ . ' or ID of address.',
	...
	public function setOrderDoneHTML($html)
	{
		$this->order_done_html = $html;
		$this->setCartIntoSession();
	}

	public function getOrderDoneHTML()
	{
		return $this->order_done_html;
	}

	public function isOrderDoneHTML()
	{
		return ($this->order_done_html !== NULL);
	}
